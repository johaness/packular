...
class Target(object): # silence pyflakes
    ...
    def __init__(self, cfg, sect):
        self.__dict__.update(
            ...
            html_preprocessor=cfg.get(sect, 'html_preprocessor', None),
            combine_partial=cfg.get(sect, 'combine_partial', None),
...
def read_config(config_file, defaults, ignore_colon):
    ...
    targets = dict((sect[PFX_LEN:], Target(cfg, sect))
                   for sect in cfg.sections() if sect.startswith('target:'))

    for name, target in targets.items():
        section = ':'.join(['target', name])
        extra_opts = [opt for opt in cfg.options(section) if opt not in target.__dict__]

        target.__dict__['context'] = defaultdict(dict)
        for opt in extra_opts:
            target.__dict__['context'].update({opt: cfg.get(section, opt)})
...
def partials(filename, urls, html_preprocessor):
    """Read HTML templates and convert into angular $templateCache call.
    Optionally preprocess HTML.
    """

    def preprocess_html(html_file, html_preprocessor):
        return check_output(shlex.split(html_preprocessor),
                            stdin=html_file, stderr=STDOUT)

    def html2js(url, html_preprocessor):
        """convert HTML into JavaScript string. Optionally preprocess HTML."""
        with open('./' + url) as f:
            if html_preprocessor:
                data = preprocess_html(f, html_preprocessor)
            else:
                data = f.read()
    ...
    tmpls = [TMPL % html2js(url, html_preprocessor) for url in urls]
...
def build(target):
    ...
    if target.combine_partial:
        print("  Partials:", target.combine_partial)
        partials(target.combine_partial,
                 target.url_html, target.html_preprocessor)
